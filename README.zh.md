...
所有图用 Times New Roman、300 dpi。数据**全部来自真实 SWE-bench Pro**（`sweap_eval_full_v2.jsonl`，731 个真实 PR 实例，已随仓库放在 `data/`）：每个 instance 的**共改文件集合**与**符号**解析自官方 gold patch，仓库/路径/测试均真实。建模部分（已显式标注、可替换）：单步 hint 噪声、System2 成功率、干扰步（用同仓库其它真实 instance 的真实文件混入)。端到端复现：

```bash
node figures/gen_fig_data.mjs        # 从真实数据采集 -> figures/fig_data.json
...
**驱动头条结果的 10 个真实实例**（`selected10.json`）：element-web（6 文件）、qutebrowser（5）、NodeBB（5）、openlibrary（4）、vuls（4）、tutanota（5）、flipt（3）、webclients（3）、ansible（2）、teleport（2）。
