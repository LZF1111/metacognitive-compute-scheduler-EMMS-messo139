...
**两层已整合进同一条在线 MCP 竞价；第三层为独立 beta 模块**（§6）：
...
| **中观集群层** (`clusterAgent`) | 把强耦合的步骤当成一个子目标 | *整个子目标该不该锁定深思，而不是被逐步的噪声提示骗过去？* | 🧪 独立 beta（未接入 MCP，无 cluster 工具，无技能检索） |

元认知分配算力、技能记忆提供已验证的内容，二者在同一场拍卖里竞价（`decide_step` / `report_outcome`）。中观集群层目前是一个**独立 beta 原型**：它单独包裹元认知层验证"子目标锁定"假设，但**尚未接入这条在线 MCP 竞价**，也没有对应的 MCP 工具或技能检索。你可以只用第一层，忽略其余。
...
> **诚实边界。**本地 embedding 是 **64 维 FNV-1a token 哈希**——对真实错误/栈/补丁文本的*词法*相似检索，一个零依赖起点，**不是**训练过的语义代码 embedding（检索向量只取"决策前可见的错误/栈"，事后 patch 摘要不混入，避免稀释错误匹配）。技能可复用度由**受信任的执行器**（退出码、测试命令、commit/patch hash）写入，而非 agent 自报 `outcome=1`。中观集群层（`clusterAgent.mjs`）目前是**独立 beta 模块**：它单独包裹元认知层、**未接入这条在线 MCP 竞价**，没有 cluster 工具，也没有技能检索；且**尚未**自动发现簇边界（需外层 agent 显式调 `startCluster()`）。
...
- **两层已整合，第三层是独立 beta。** 元认知（`selfModel`）与技能记忆（`skillMemory`）在**同一条在线 MCP 竞价**里出价；介尺度簇（`clusterAgent`）是**独立 beta 原型**——尚未接入这条竞价，没有 cluster 工具，也没有技能检索。说"三层已在同一条在线拍卖里竞价"会夸大当前的整合程度。
