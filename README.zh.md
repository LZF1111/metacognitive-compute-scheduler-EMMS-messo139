...
## 6. 技能记忆层：学习领域经验（而非替代 skill）

本 README 早期版本声称原型库“能替代写死的 skill”——那是一个过度声明，现**收回**。元认知调度器学的是*该想多深*——它**不**学 *pytest 里的 `ScopeMismatch` 长什么样、怎么修*。那些领域内容住在专门的**技能记忆层**（`skillMemory.mjs`），两者是**协作**而非一方替代另一方。
